...
st.set_page_config(page_title="Alfred v2.3", page_icon="🤖", layout="wide")
...
# En-tête
st.title("🤖 Alfred — version 2.3 (mémoire contextuelle pondérée)")
...
from memoire_alfred import (
    ...
    autosave_heartbeat,
    confirm_delete,
    search_contextual_memories,  # <-- nouvelle fonction v2.3
)
...
if prompt:
    ...
    # 1️⃣ Commandes mémoire (souviens-toi, oublie, rappelle-toi…)
    ...
    # 2️⃣ Recherche de contexte mémoire pondéré (v2.3)
    context_memories = search_contextual_memories(prompt, top_k=5)
    contextual_text = ""

    if context_memories:
        contextual_text = "\n".join(
            [f"- {m.get('texte','')}" for m in context_memories if m.get("texte")]
        )
        st.sidebar.markdown("### 🧠 Mémoire utilisée")
        st.sidebar.info(contextual_text)

        # Reformulation du contexte (intégrée au prompt)
        context_intro = (
            "Voici quelques éléments de ta mémoire susceptibles de t’aider à raisonner :\n"
            f"{contextual_text}\n\n"
        )
    else:
        context_intro = ""

    # 3️⃣ Routeur (Drive, modules spécialisés, etc.)
    reponse = router(prompt)
    if reponse is None:
        # 4️⃣ Fallback LLM — ajout du contexte mémoire dans le prompt
        if fichier:
